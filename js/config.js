...
export const APP_VERSION = 'v1.1.11';  // bump this after each push
export const CUR = 'Rs ';
...
export const OCR_PROMPT = 'You are a receipt/invoice parser for a construction project in a rupee currency. Read EVERY visible line on the bill. Reply ONLY with strict JSON, no markdown, no prose. Schema: {"seller":string,"date":"YYYY-MM-DD","receipt":string,"category":string,"total":number,"lines":[{"item":string,"qty":number|string,"rate":number,"amount":number}]}. Rules: "seller" = shop/company name at the top. "date" = bill date as YYYY-MM-DD (not today unless printed). "receipt" = invoice/bill/receipt number. "lines" = every purchased item/service row, not headers, not subtotals. "qty" and "rate" if printed, else empty. "amount" = that row\'s line total. "total" = grand total payable (numbers only, no currency text). "category" = exactly one of: ' + CATEGORIES.join(', ') + '. Never invent items you cannot see. If a field is unreadable use "" or [].';
