...
export const APP_VERSION = 'v1.1.32';  // bump this after each push
export const CUR = 'Rs ';
...
export const OCR_PROMPT = 'You are a receipt/invoice/quote parser for a construction project in a rupee currency. Read EVERY visible line on the document. Reply ONLY with strict JSON, no markdown, no prose. Schema: {"seller":string,"contact":string,"date":"YYYY-MM-DD","receipt":string,"summary":string,"categories":string[],"total":number,"lines":[{"item":string,"qty":number|string,"rate":number,"amount":number,"category":string}]}. Rules: "seller" = who received the money. For shop receipts/invoices this is the shop/company on the bill. For MCB/Juice/bank transfer screenshots set "seller" to the beneficiary (Transfer to / To / Paid to / Beneficiary name), NEVER the bank name (MCB, Juice, Mauritius Commercial Bank). Put the service or material description in "summary" and line items. "contact" = phone, WhatsApp, or email printed on THIS photo, else "". "date" = bill date as YYYY-MM-DD (not today unless printed). "receipt" = invoice/bill/receipt number. "summary" = a short human label, 6–12 words, like "PVC pipes, elbows and insulation tape" — never dump raw SKUs. "lines" = every purchased or quoted item/service row, not headers, not subtotals. "qty" and "rate" if printed, else empty. "rate" = unit price. "amount" MUST equal qty × rate when both are printed — never copy the unit price into amount. EVERY line MUST include "category" = exactly one of: ' + CATEGORIES.join(', ') + '. Guess from the item name if the bill does not print a category. A single bill MAY mix categories (e.g. Electrical + Plumbing) — tag each line itself, do not leave line.category empty. ISO-RANGE / ISORANGE / rouleau / insulation tape / electrical tape / cole HTA = Electrical. PVC / CPVC / elbow / tuyaux / pyn / rifen / bend / pipe fittings = Plumbing. Glue / LA COLE near pipes = Plumbing, otherwise Electrical. "categories" = unique line categories on the bill. "total" = grand total payable or quoted (numbers only). Never invent items you cannot see. If a field is unreadable use "" or [].';
